...
const db = new Database(path.join(dbDir, "skillsync.db"));
...
try {
  ...
  console.error("⚠️ Error initializing Gemini:", error.message);
}
...
async function analyzeWithGemini(resumeText, jobDescription) {
  ...
  try {
    ...
    const prompt = `You are a strict hiring manager conducting a technical skills assessment. 
...
Return your response in this EXACT JSON format:
{
  ...
  "recommendations": [
    ...
    try {
      ...
      if (!analysis.matchPercentage || !Array.isArray(analysis.missingSkills)) {
  ...
  try {
    const stmt = db.prepare(`
      INSERT INTO skill_gaps (
        student_id, student_name, department, academic_year, job_role, company_name,
